...
//DKS - modified to allow spaces in series filenames in the rc file
...
static char *readconfigfile(fileinfo *file, gameseries *series)
{
	...
	i = 0;
	while (!((tmpstr[i] == '\n') || (tmpstr[i] == '\r') || (tmpstr[i] == '\0')) && i < (sizeof(datfilename) - 1)) {
		datfilename[i] = tmpstr[i];
		++i;
	}
	datfilename[i] = '\0';
...
//DKS - modified to make there be no difference between upper and lower case filenames
...
static int gameseriescmp(void const *a, void const *b)
{
    ...
    strupr(tmpstr1,((gameseries*)a)->name);
    strupr(tmpstr2,((gameseries*)b)->name);
